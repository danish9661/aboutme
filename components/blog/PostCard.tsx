...
export default function PostCard({ post, index, featured = false }: PostCardProps) {
  ...
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group block h-full overflow-hidden rounded-2xl border border-line bg-surface transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_22px_55px_-20px_rgba(124,58,237,0.5)] motion-reduce:hover:translate-y-0"
    >
      ...
      <div className={featured ? "p-7 sm:p-9" : "p-6"}>
        <div className="flex items-center gap-3">
          <span
            className={`flex shrink-0 items-center justify-center rounded-xl font-mono font-semibold text-white shadow-sm transition-transform duration-300 group-hover:scale-105 group-hover:-rotate-3 motion-reduce:transform-none ${
              featured ? "h-12 w-12 text-[14px]" : "h-10 w-10 text-[12px]"
            }`}
